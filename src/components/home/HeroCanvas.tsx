...
export function HeroCanvas() {
  ...
  return (
    <div
      ...
      onClick={handleClick}
      role="button"
      aria-label="Click to mutate the composition"
      title="Click to mutate"
      className="group relative aspect-square w-full cursor-pointer overflow-hidden rounded-3xl border border-border shadow-[0_1px_2px_rgba(0,0,0,0.04),0_24px_64px_-16px_rgba(0,0,0,0.25)] transition-transform duration-500 hover:scale-[1.01]"
    ...
    >
      ...
      />
      <div className="pointer-events-none absolute bottom-4 right-4 rounded-full bg-black/40 px-3 py-1 text-xs font-medium text-white opacity-0 backdrop-blur-sm transition-opacity duration-200 group-hover:opacity-100">
        Click to mutate
      </div>
    </div>
  )
