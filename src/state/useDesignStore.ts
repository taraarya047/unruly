...
export const useDesignStore = create<DesignStoreState>((set, get) => {
  ...
  return {
    ...initial,
    locks: { geometry: false, palette: false },
    history: [initial],
    historyIndex: 0,
    advancedMode: false,
    ...
    randomizeNew: () => {
      set((state) => {
        ...
        const generator = generatorRegistry.get(generatorId)!
        const parameters = state.locks.geometry ? state.parameters : { ...generator.defaultParameters }
    ...
    randomizeEvolve: () => {
      set((state) => {
        ...
        const [variation] = generateVariations(generator, state.parameters, 1, 0.18)
        const snapshot: DesignSnapshot = {
